...
pady=5, sticky=tk.W)
ttk.Entry(control_frame, textvariable=self.stock_symbol, width=10).grid(row=0,
column=1, padx=5, pady=5)

ttk.Label(control_frame, text="Time Period:").grid(row=0, column=2, padx=5,
pady=5, sticky=tk.W)
ttk.Combobox(control_frame, textvariable=self.time_period, values=["1d", "5d",
"1mo", "3mo", "6mo", "1y"], width=5).grid(row=0, column=3, padx=5, pady=5)

ttk.Label(control_frame, text="Refresh (sec):").grid(row=0, column=4, padx=5,
pady=5, sticky=tk.W)
ttk.Spinbox(control_frame, textvariable=self.refresh_interval, from_=10,
to=3600, width=5).grid(row=0, column=5, padx=5, pady=5)
...
command=self.fetch_stock_data).grid(row=0, column=7, padx=5, pady=5)
...
self.info_text = tk.Text(data_left, wrap=tk.WORD, height=10, font=('Consolas',
10))
self.info_text.pack(fill=tk.BOTH, expand=True, pady=(0, 10))

# Left Frame - History Table
columns = ("Date", "Open", "High", "Low", "Close", "Volume")
self.history_tree = ttk.Treeview(data_left, columns=columns, show="headings",
height=10)
...
# Basic info
info = stock.info
display_text = f"Stock: {info.get('longName', 'N/A')}
({self.stock_symbol.get()})\n"
display_text += f"Current Price: ${current_price:.2f}\n"

# Market info
display_text += f"Market Cap: ${info.get('marketCap', 'N/A'):,}\n"
display_text += f"52 Week Range: ${info.get('fiftyTwoWeekLow', 'N/A')} -
${info.get('fiftyTwoWeekHigh', 'N/A')}\n"
display_text += f"PE Ratio: {info.get('trailingPE', 'N/A')}\n"
display_text += f"Dividend Yield: {info.get('dividendYield', 'N/A')*100 if
info.get('dividendYield') else 'N/A'}%\n"
...
display_text += f"Change: {'+' if change >= 0 else ''}{change:.2f} ({'+' if
percent_change >= 0 else ''}{percent_change:.2f}%)\n"
...
# Plot the data
self.stock_data['Close'].plot(ax=self.ax, title=f"{self.stock_symbol.get()}
Stock Price", grid=True)
